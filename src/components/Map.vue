<template>
      ...
      <div class="panel">
        ...
        <div class="block">
          ...
          <div class="row">
            <div class="cell">
              ...
              <div class="mini">
                <button class="btn" @click="useMyLocation('from')">Use my location</button>
                <button class="btn" @click="useMapClick('from')">{{ picking==='from'?'Click on map…':'Pick on map' }}</button>
              </div>
            </div>
            <div class="cell">
              <label>To</label>
              ...
              <div class="mini">
                <button class="btn" @click="useMyLocation('to')">Use my location</button>
                <button class="btn" @click="useMapClick('to')">{{ picking==='to'?'Click on map…':'Pick on map' }}</button>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="cell">
              <label>&nbsp;</label>
              <div class="mini">
                <button class="btn" :disabled="!from || !to" @click="swap">Swap</button>
                <button class="btn primary" :disabled="!from || !to" @click="buildRoute">Build Route</button>
              </div>
            </div>
          </div>
        ...
        <div class="block">
          ...
          <div class="row">
            ...
            <div class="cell">
              <label>&nbsp;</label>
              <button class="btn" @click="searchPOI">Search POI</button>
            </div>
          </div>
  ...
  // state
  const map = ref(null)
  const profile = ref('driving') // driving / walking / cycling
  const from = ref(null) // {lng, lat, name}
  const to = ref(null)
  ...
  const picking = ref(null) // 'from' | 'to' | null
  ...
  const poiCenter = ref('map') // 'map' | 'from' | 'to'
  ...
  function swap(){
    ...
    to.value = tmp
    // swap markers
    const fLL = fromMarker.value?.getLngLat()
    const tLL = toMarker.value?.getLngLat()
    if (fLL && tLL){
      fromMarker.value.setLngLat(fLL)
      toMarker.value.setLngLat(tLL)
    }
    clearRoute()
  }
  ...
  function useMapClick(which){
    picking.value = picking.value === which ? null : which
  }
  function useMyLocation(which){
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      if (which === 'from'){
        ...
        fromMarker.value = makeMarker(ll, '#16a34a')
      }else{
        to.value = ll
        toMarker.value?.remove()
        toMarker.value = makeMarker(ll, '#ef4444')
      }
  ...
  async function searchPOI(){
    ...
    // Search by category via geocoding 'poi'
    // 限制 15 个，再用 turf 过滤半径
    ...
    if (filtered.length){
      const b = new mapboxgl.LngLatBounds()
      filtered.forEach(f => b.extend(f.geometry.coordinates))
      b.extend(center)
      map.value.fitBounds(b, { padding: 60, duration: 600 })
    }
  }
  ...
  onMounted(() => {
    ...
    gcFrom.addTo(fromBox.value)
    gcTo.addTo(toBox.value)
  
    gcFrom.on('result', (e) => {
      const [lng, lat] = e.result.center
      from.value = { lng, lat, name: e.result.place_name }
      fromMarker.value?.remove()
      fromMarker.value = makeMarker([lng, lat], '#16a34a')
      if (to.value) fitBoundsToPoints([from.value, to.value]); else m.flyTo({ center: [lng,lat], zoom: 13 })
      clearRoute()
    })
    gcTo.on('result', (e) => {
      const [lng, lat] = e.result.center
      ...
      toMarker.value?.remove()
      toMarker.value = makeMarker([lng, lat], '#ef4444')
      if (from.value) fitBoundsToPoints([from.value, to.value]); else m.flyTo({ center: [lng,lat], zoom: 13 })
      clearRoute()
    })
  ...
  .geocoder :deep(.mapboxgl-ctrl-geocoder){min-width:100%;max-width:100%}
